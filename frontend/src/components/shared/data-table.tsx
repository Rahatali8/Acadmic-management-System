...
export function DataTable<T extends { id: number }>({
  ...
  if (isLoading) {
    return (
      <div className="w-full bg-white rounded-lg shadow border border-[#a3cef1] p-4">
          ...
          >
            {onSelectionChange && (
              <div className="flex items-center justify-between p-2 border-b border-gray-100 bg-gray-50/50">
                <button onClick={(e) => { e.stopPropagation(); toggleOne(item.id); }} className="p-1 rounded text-[#6096ba]">
                   {selectedIds.includes(item.id) ? <CheckSquare className="h-6 w-6" /> : <Square className="h-6 w-6" />}
                </button>
                <div className="text-[10px] font-bold text-[#274c77] uppercase">ID: {item.id}</div>
              </div>
            )}
        ...
        <Table>
          <TableHeader>
            <TableRow className="bg-[#274c77] hover:bg-[#274c77]">
              {onSelectionChange && (
                <TableHead className="w-10 px-3 py-2 text-white">
                  <button onClick={toggleAll} className="p-1 hover:bg-white/10 rounded transition-colors" title="Select All">
          ...
          <TableBody>
            {data.map((item) => (
              <TableRow key={item.id} className={`hover:bg-gray-50 text-gray-900 ${selectedIds.includes(item.id) ? 'bg-blue-50/50' : ''}`}>
                {onSelectionChange && (
