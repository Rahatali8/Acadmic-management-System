...
// Custom theme palette matching others
const THEME_COLORS = [
  "#274c77", // Deep Blue
  "#2a9d8f", // Teal
  "#e9c46a", // Muted Gold
  "#f4a261", // Terra Cotta
  "#e76f51", // Burnt Orange
  "#264653", // Dark Green-Blue
]

export function ReligionChart({ data, isLoading }: ReligionChartProps) {
  if (isLoading) {
    return (
      <Card className="flex flex-col h-full shadow-lg border-t-4 border-t-[#274c77] hover:shadow-xl transition-all duration-300">
          ...
          <PieChart>
            ...
            <defs>
              {chartData.map((entry, index) => (
                <linearGradient id={`gradient-rel-${index}`} key={`gradient-rel-${index}`} x1="0" y1="0" x2="0" y2="1">
                  <stop offset="0%" stopColor={entry.fill} stopOpacity={0.8} />
                  <stop offset="95%" stopColor={entry.fill} stopOpacity={1} />
                </linearGradient>
              ))}
              <filter id="shadow-rel" height="130%">
