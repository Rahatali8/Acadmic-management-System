  ...
  const validateCurrentStep = () => {
    ...
    for (const field of required) {
      ...
      if (Array.isArray(value)) {
        if (value.length === 0) isInvalid = true
      } 
      // If the field is contact_number (+XX XXXXXXXXXX)
      else if (field === 'contact_number') {
        const parts = (value || "").split(" ")
        const num = parts.length > 1 ? parts[1] : ""
        if (!num || num.trim() === "") isInvalid = true
      }
      ...
      if (isInvalid) {
        invalid.push(field)
        newFieldErrors[field] = `${field.split('_').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' ')} is required`
      }
    }
