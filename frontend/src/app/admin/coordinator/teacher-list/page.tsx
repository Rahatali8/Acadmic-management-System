...
import { Users, Search, Eye, Edit, User, Mail, Phone, GraduationCap, MapPin, Calendar, Award, Plus } from "lucide-react"
...
function CoordinatorTeacherListContent() {
  ...
  const handleEdit = async (teacher: any) => {
    const role = getCurrentUserRole();
    if (role === 'teacher' || role === 'student' || role === 'guest') {
      toast.error("Unauthorized: You do not have permission to edit teacher information.");
      return;
    }
    try {
  ...
  const handleEditSubmit = async () => {
    ...
    try {
      ...
      if (response.ok) {
        toast.success(`✅ Success! Teacher ${editFormData.full_name || editingTeacher.name} has been updated successfully!`);
