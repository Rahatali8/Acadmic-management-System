...
function CoordinatorStudentListContent() {
  ...
  useEffect(() => {
    ...
    fetchStudents()
  }, [currentPage, pageSize, search, selectedShift, selectedGrade, selectedSection, selectedLevel, selectedGender, selectedOrdering])
  ...
  const columns = [
    {
      ...
      render: (student: StudentRow) => (
        <div className="flex flex-col gap-0.5">
          ...
          <p className="text-[11px] sm:text-xs text-gray-500 font-medium italic">
            {student.gender?.toLowerCase() === 'female' ? 'D/O' : 'S/O'} {student.father_name}
          </p>
        </div>
      ),
    },
          ...
          <div className="flex flex-col gap-3 sm:gap-4 mb-3 sm:mb-4">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 mb-2">
              <div className="flex items-center gap-2">
                <div className="p-2 bg-[#274c77]/10 rounded-lg">
                  <Search className="h-5 w-5 text-[#274c77]" />
                </div>
                <div>
                  <h3 className="text-lg font-bold text-[#274c77]">Search Filter</h3>
                  <p className="text-xs text-gray-500">Filter students by various criteria</p>
                </div>
              </div>
              <Button
                onClick={handleClearFilters}
                variant="outline"
                className="h-10 border-[#a3cef1] text-[#274c77] hover:bg-[#274c77]/5 transition-all"
              >
                <RefreshCcw className={`h-4 w-4 mr-2 ${isClearing ? 'animate-spin' : ''}`} />
                Clear Filters
              </Button>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3 sm:gap-4">
              ...
              <div className="space-y-1.5">
                <Label className="text-xs font-semibold text-gray-700">Student Search</Label>
                <div className="relative">
                  ...
                  <Input
                    ...
                    className="pl-9 h-11 border-[#a3cef1] rounded-xl focus:ring-[#274c77]"
                  />
                </div>
              </div>
              ...
              {/* Shift Filter */}
              <div className="space-y-1.5 ">
                <Label className="text-xs font-semibold text-gray-700">Academic Shift</Label>
                <Select value={selectedShift} onValueChange={setSelectedShift} >
                  <SelectTrigger className="h-11 border-[#a3cef1] rounded-xl text-[#274c77]">
                    <SelectValue placeholder="Select shift" />
                  </SelectTrigger>
                  <SelectContent >
                    <SelectItem value="all" className="w-80">All Shifts</SelectItem>
                ...
                <Select value={selectedLevel} onValueChange={setSelectedLevel}>
                  ...
                  <SelectContent>
                    ...
                    {levels.map((level: any) => {
                      const name = (level.name || level.level_name || "").trim();
                      return (
                        <SelectItem key={level.id} value={name}>
                          {name}
                        </SelectItem>
                      );
                    })}
              ...
              {/* Grade Filter */}
              <div className="space-y-1.5">
                <Label className="text-xs font-semibold text-gray-700">Grade/Class</Label>
                <Select
                  value={selectedGrade}
