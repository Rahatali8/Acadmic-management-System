...
import { getFilteredTeachers, getAllCampuses, getGrades, authorizedFetch, API_ENDPOINTS } from "@/lib/api";
import { DataTable, PaginationControls, ListFilters } from "@/components/shared";
import { LoadingSpinner } from "@/components/ui/loading-spinner";
import { User, Mail, GraduationCap, MapPin, Award, RefreshCcw, Search, Plus } from 'lucide-react';
...
export default function TeacherListPage() {
  ...
  useEffect(() => {
    if (userRole === 'principal' && allGrades.length > 0) {
      const selectedShift = filters.shift;

      if (!selectedShift || selectedShift === 'both') {
        // Show all grades when shift is 'both' or not selected
        setFilteredGrades(allGrades);
      } else {
        // Filter grades by shift
        const filtered = allGrades.filter((grade: any) =>
          grade.shift === selectedShift || grade.shift === 'both'
        );
        setFilteredGrades(filtered);
      }

      // Reset grade filter if the selected grade is no longer in filtered list
      if (filters.grade) {
        const gradeStillExists = allGrades.some((g: any) =>
          g.id.toString() === filters.grade &&
          (g.shift === selectedShift || g.shift === 'both' || !selectedShift || selectedShift === 'both')
        );
        if (!gradeStillExists) {
  ...
  }, [filters.shift, allGrades, userRole]);
      ...
      if (userProfile?.campus) {
        ...
        if (role === 'principal' && cId) {
          ...
          try {
            ...
            setFilteredGrades(gradesList); // Initially show all valid for campus
          } catch (err) {
  ...
  const handleEditSubmit = async () => {
    ...
    try {
      ...
      if (response.ok) {
        toast.success(`Teacher ${editFormData.full_name || editingTeacher.full_name} has been updated successfully!`);
        setShowEditDialog(false);
        setEditingTeacher(null);
        setEditFormData({});
        ...
        toast.error(`Error updating teacher: ${response.status} - ${errorData}`);
      }
    } catch (error) {
          ...
          <div className="min-w-0 flex-1">
            ...
            {teacher.father_name && (
              <div className="text-[10px] sm:text-xs text-gray-500 italic">
                S/O or W/O: {teacher.father_name}
              </div>
            )}
    ...
    <div className="p-2 sm:p-3 w-full max-w-full overflow-hidden">
      <div className="mb-3 sm:mb-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        ...
        {getPermissions(userRole).canAddTeacher && (
          <Button
            ...
            className="w-full sm:w-auto bg-[#274c77] hover:bg-[#6096ba] text-white font-bold py-2 px-4 rounded-xl shadow-lg transition-all duration-300 transform hover:scale-[1.02] active:scale-95 flex items-center justify-center gap-2"
          >
      ...
      <div className="bg-white rounded-lg shadow p-2 sm:p-3 mb-3 w-full" style={{ borderColor: '#a3cef1' }}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2 mb-3">
          ...
          <div className="sm:col-span-1 lg:col-span-1">
            ...
            <div className="relative">
              <input
                ...
                className="w-full px-3 py-1.5 pl-9 text-sm border rounded-md focus:outline-none focus:ring-2"
          ...
          {/* Campus Filter - For principal: show only their campus (disabled), others: show all */}
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">
              Campus
            </label>
            {userRole === 'principal' ? (
              <select
                ...
                disabled
                className="w-full px-2 py-1.5 text-sm border rounded-md bg-gray-100 cursor-not-allowed"
                style={{ borderColor: '#a3cef1' }}
              >
                <option value={userCampusId?.toString() || ""}>
                  {userCampus || 'Your Campus'}
                </option>
              </select>
            ) : (
              <select
          ...
          {/* Shift Filter - For principal: based on campus shift_available */}
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">
              Shift
            </label>
            {userRole === 'principal' ? (
              // Principal's shift filter based on campus shift
              campusShift === 'both' ? (
                // Campus has both shifts - show dropdown
                <select
                  value={filters.shift}
                ...
                // Campus has single shift - show disabled with that shift
                <select
                  value={campusShift}
                  disabled
                  className="w-full px-2 py-1.5 text-sm border rounded-md bg-gray-100 cursor-not-allowed"
                ...
                >
                  <option value={campusShift}>
                    {campusShift ? campusShift.charAt(0).toUpperCase() + campusShift.slice(1) : 'N/A'}
                  </option>
                </select>
              )
            ) : (
              // Non-principal: show all shifts
              <select
                value={filters.shift}
                onChange={(e) => handleFilterChange('shift', e.target.value)}
                className="w-full px-2 py-1.5 text-sm border rounded-md focus:outline-none focus:ring-2"
          ...
          {/* Grade Filter (for Principal) OR Status Filter (for others) */}
          {userRole === 'principal' ? (
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">
                Grade
              ...
              >
                ...
                {filteredGrades.map((grade: any) => (
                  <option key={grade.id} value={grade.id}>
                    {grade.name} ({grade.shift ? grade.shift.charAt(0).toUpperCase() + grade.shift.slice(1) : ''})
                  </option>
                ))}
              </select>
            </div>
      ...
      <DataTable
        ...
        allowView={getPermissions(userRole).canViewTeachers} // Wait, View action on individual teacher
        allowEdit={getPermissions(userRole).canEditTeacher}
        allowDelete={getPermissions(userRole).canDeleteTeacher}
      />
