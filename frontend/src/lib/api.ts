      ...
      if (typeof err === 'object' && err !== null) {
        return Object.entries(err)
          .map(([key, val]) => {
            ...
            // If the key is 'non_field_errors' or 'detail' or 'error', usually just return the value
...
export async function getFilteredStudents(params: {
  ...
  shift?: string;
  classroom?: number;
  level?: number | string; // Added level filter
  ordering?: string;
}): Promise<{
  count: number;
  next: string | null;
...
export async function getFilteredTeachers(params: {
  ...
  grade?: string | number; // Added grade filter
  ordering?: string;
}): Promise<{
  count: number;
  next: string | null;
...
export async function getAlumniStudents(campusId?: number) {
  try {
    ...
    let url = `${API_ENDPOINTS.STUDENTS}?current_grade=Alumni&is_draft=false&is_deleted=false&_t=${timestamp}`;
    if (campusId) {
      url += `&campus=${campusId}`;
    }
    const data = await apiGet(url);
...
export async function calculatePositions(classroomId?: number) {
  try {
