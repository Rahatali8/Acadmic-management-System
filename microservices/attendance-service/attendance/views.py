...
from .models import Attendance, StudentAttendance, Weekend, ZKTecoDevice, ZKTecoEmployeeMapping, StaffAttendance, EmployeeShiftTiming
...
def get_student_monthly_attendance(request, student_id):
    ...
    # Extract calendar year from academic year based on month
    # Standard academic year 2024-25: 
    ...
    if month_num <= 3: # Jan, Feb, Mar are in the second half of academic year
        calendar_year += 1
        
    classroom = student.classroom
    if not classroom:
        return Response({'days_present': 0, 'total_days': 0})
    ...
    total_working_days = Attendance.objects.filter(
        ...
        is_deleted=False
    ).exclude(status='draft').count() # Only count non-draft attendance
    ...
    days_present = StudentAttendance.objects.filter(
        ...
        Q(status='present') | Q(status='late') | Q(status='leave') # Count present, late, and leave as 'attended' or 'present' for result purposes? 
        # Actually usually 'leave' is not present but marked. 
        # Let's count present and late only for 'Days Present'
    ).exclude(attendance__status='draft').count()
